...
from yir.relay.converter import TorchScriptConverter
from yir.relay.graph import Grapher
from yir.relay.ncnn_emitter import NCNNEmitter
from yir.relay.ncnn_param import NCNNParamDispatcher
from yir.relay.optimizer import GraphOptimization
from yir.relay.parser import H5dfParser
...
def main():
    ...
    parser.add_argument("--input_file", type=str, required=True, help="Input h5df file")
    parser.add_argument("--output_dir", type=str, default="", help="Output file dir")
    parser.add_argument("--plot_graph", action="store_true", help="Virtualize graph.")
    parser.add_argument("--debug", action="store_true", help="Run accuracy debug.")
    ...
    TorchScriptConverter().parse_keras_graph(
        keras_graph, ncnn_graph, NCNNParamDispatcher()
    )
    ...
    if args.output_dir != "":
        ...
        emitter.emit_param(
            os.path.join(args.output_dir, Path(args.input_file).stem + ".param"),
            graph_seq,
        )

        print("\tEmitting binary...")
        emitter.emit_binary(
            os.path.join(args.output_dir, Path(args.input_file).stem + ".bin"),
            graph_seq,
        )
