...
struct XLayer {
  ...
  bool is_input() {
    return input_types_.find(xtype[0]) != input_types_.end();
  }

  // TOPS & BOTTOMS FUNCTIONALITY

  bool has_top(const std::string& top_name) {
    const std::string top_name_str = yir::stringify(top_name);
    return std::find(tops.begin(), tops.end(), top_name_str) != tops.end();
  }

  void add_top(const std::string& top_name) {
    tops.push_back(yir::stringify(top_name));
  }

  void remove_top(const std::string& top_name) {
    const std::string top_name_str = yir::stringify(top_name);
    for (std::vector<std::string>::iterator it = tops.begin(); it != tops.end(); ++it) {
      if (*it == top_name_str) {
        tops.erase(it);
        break;
      }
  ...
  bool has_bottom(const std::string& bottom_name) {
    const std::string bottom_name_str = yir::stringify(bottom_name);
    return std::find(bottoms.begin(), bottoms.end(), bottom_name_str) != bottoms.end();
  }

  void add_bottom(const std::string& bottom_name) {
    bottoms.push_back(yir::stringify(bottom_name));
  }

  void remove_bottom(const std::string& bottom_name) {
    const std::string bottom_name_str = yir::stringify(bottom_name);
    for (std::vector<std::string>::iterator it = bottoms.begin(); it != bottoms.end(); ++it) {
      if (*it == bottom_name_str) {
        bottoms.erase(it);
        break;
      }
    }
