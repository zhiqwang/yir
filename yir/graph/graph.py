...
"""
Module for Graph data structure
"""
...
class Graph:
    ...
    """
    The Graph data structure for storing the model graph, accessing properties
    ...
    @classmethod
    def _from_xgraph(cls, _xgraph: lpx.Graph):
        xg = Graph.__new__(cls)
        xg._xgraph = _xgraph
        xg.init()
        return xg
    ...
    def _reset(self):
        ...
        Graph based on the current pydot graph
        """
    ...
    @property
    def meta_attrs(self):
        return AttrDict(self._xgraph.meta_attrs)

    @meta_attrs.setter
    def meta_attrs(self, d: dict):
        _xattr_dict = AttrDict(lpx.XAttrMap())
    ...
    def get(self, layer_name):
        # type: (str) -> Layer
        """
        Return an Layer object by name
        """
        return Layer._from_xlayer(self._xgraph.get(layer_name))
    ...
    def add(self, X):
        # type: (Layer) -> None
        """
        Add the provided Layer object to the graph
        """
    ...
    def remove(self, layer_name):
        # type: (str) -> None
        """
        Remove the layer with given name and link the bottom and top layers.
        """
    ...
    def __setup_targets_for_X(self, X: Layer) -> None:
        """
        Setup the supported targets for the provided Layer
        """
        # Check with registered targets, which device can execute
        #   this Layer and add those targets to the Layer device
        #   attribute
        pass
        # X.targets = []
        ...
        # for device in Graph.target_registry.get_targets():
    ...
    def __contains__(self, layer_name: str) -> int:
        """
        Reports whether a layer with given name exists in the Graph
        """
        return layer_name in self._xgraph
